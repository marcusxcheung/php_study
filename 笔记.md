...
2. 只要知道5個左右  主要有
	(1) $_SERVER['REMOTE_IP'] 獲取訪問者ip地址
	(2) $_SERVER['SERVER_IP'] 獲取服務器所在的ip地址
	(3) $_SERVER['SERVER_NAME'] 獲取服務器的名字 其實就是servername
	(4) $_SERVER['DOCUMENT_ROOT'] 獲取服務器的真實物理地址
	(5) $_SERVER['PHP_SELF'] 獲取當前網頁地址 不含域名部分
	(6) $_SERVER['QUERY_STRING'] 獲取所有get數據
